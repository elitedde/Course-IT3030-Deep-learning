...
class VerificationNet:
    ...
    def train(self, generator: StackedMNISTData, epochs: int = 10) -> bool:
        ...
        if self.force_relearn or self.done_training is False:
            ...
            # Fit model
            self.model.fit(x=x_train, y=y_train, batch_size=1024, epochs=40,
...
if __name__ == "__main__":
    ...
    # net = VerificationNet(force_learn=True, file_name='./models/mono_binary_complete.h5')
...
#--------------------------------------#
    ...
    # net = VerificationNet(force_learn=True, file_name='./models/color_binary_complete.h5')
    # net.train(generator=gen, epochs=30)
    #
    # # I have no data generator (VAE or whatever) here, so just use a sampled set
    # img, labels = gen.get_random_batch(training=True,  batch_size=25000)
...
#--------------------------------------#
    ...
    # net = VerificationNet(force_learn=True, file_name='./models/mono_float_complete.h5')
    # net.train(generator=gen, epochs=30)
    #
    # # I have no data generator (VAE or whatever) here, so just use a sampled set
    # img, labels = gen.get_random_batch(training=True,  batch_size=25000)
...
#---------------------------------------#
    ...
    net = VerificationNet(force_learn=False, file_name='./models/mono_binary_missing.h5')
    net.train(generator=gen, epochs=30)
...
#-------------------------------------------#
    ...
    net = VerificationNet(force_learn=False, file_name='./models/color_binary_missing.h5')
    net.train(generator=gen, epochs=30)
    ...
    print(f"Accuracy: {100 * acc:.2f}%")
#------------------------------------------------#
    gen = StackedMNISTData(mode=DataMode.COLOR_FLOAT_MISSING, default_batch_size=2048)
    net = VerificationNet(force_learn=False, file_name='./models/color_float_missing.h5')
    net.train(generator=gen, epochs=30)
...
#----------------------------------------------#
    ...
    net = VerificationNet(force_learn=False, file_name='./models/mono_float_missing.h5')
    net.train(generator=gen, epochs=30)
